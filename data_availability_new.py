...
def process_raw_data(data, ovrd_times=None):
    ...
    for subject in subjects_list:
        ...
        base_timestamp = np.min([subject_data['sessions'][name]['relative timestamp'] for name in subject_data['sessions']])
        for name in subject_data['sessions']:
            subject_data['sessions'][name]['relative timestamp'] -= base_timestamp
...
class data_availability:
    ...
    def get_suitable_epoch_files_and_contacts(self, min_timegap_hrs, max_timegap_hrs, proc_type,
        ...
        for event_type in event_list:
            ...
            first_ok = (epoch_list['relative time'] == 0) * (epoch_list['num_epochs'] >= num_epochs)
            if not single_session:
                ...
                valid_subjects = list(set(first_subject).intersection(set(second_subject)))
                first_ok = [first_ok[i] * (epoch_list['subject'][i] in valid_subjects) for i in range(len(epoch_list))]
                second_ok = [second_ok[i] * (epoch_list['subject'][i] in valid_subjects) for i in range(len(epoch_list))]
                # intersect contact lists
    ...
    def intersect_epoch_files_and_contact_lists(self, epoch_file_lists):
        
        # make subject intersect
        subjects = set(epoch_file_lists[0]['subject'])
        for eflist in epoch_file_lists[1:]:
            subjects = subjects.intersection(set(eflist['subject']))
        for i_list, eflist in enumerate(epoch_file_lists):
            mask = [subject in subjects for subject in eflist['subject']]
            epoch_file_lists[i_list] = eflist[mask]
        
        # intersect contacts
        for subject in subjects:
            #print(subject)
            for i_list, eflist in enumerate(epoch_file_lists):
                i = int(np.argwhere(subject == eflist['subject']).squeeze())
                #print(i_list, i)
                if i_list == 0:
                    contacts = set(eflist.iloc[i]['contacts'])
                    #print(len(contacts))
                else:
                    contacts = contacts.intersection(set(eflist.iloc[i]['contacts']))
                    #print(len(contacts))
            for i_list, eflist in enumerate(epoch_file_lists):
                i = int(np.argwhere(subject == eflist['subject']).squeeze())
    ...
    def get_contact_info(self, epoch_file_list):
        
        contact_list = []
        for i, (subject, contacts) in enumerate(zip(epoch_file_list['subject'], epoch_file_list['contacts'])):
            for contact in contacts:
                contact_data = dict({'subject': subject, 'name': contact, \
                                     'location': self.__bipolar_contact_additional_data(contact, self.data[subject]['electrodes by name'])})
...
if __name__ == '__main__':
    ...
    start_times = pd.read_csv(os.path.join(IDXS_FOLDER, 'start_times.csv'))
    ...
    if READ_FILES:
        ...
        with open(os.path.join(IDXS_FOLDER, 'raw_availability_data'), 'wb') as fd:
            pickle.dump(raw_availability_data, fd)
        #assert False

    # process the files
    with open(os.path.join(IDXS_FOLDER, 'raw_availability_data'), 'rb') as fd:
        raw_availability_data = pickle.load(fd)
    availability_data = process_raw_data(raw_availability_data, ovrd_times=start_times)

    #
    with open(os.path.join(IDXS_FOLDER, 'fixed_availability_data'), 'wb') as fd:
