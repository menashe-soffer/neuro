...
if __name__ == '__main__':
    ...
    stat = []
    for subject in data_availability_obj.get_subject_list():
        contact_lists = data_availability_obj.get_sessions_for_subject(subject, event_type=event_type, epoch_subsets=epoch_subsets)
        stat.append(len(contact_lists))
        for contact_list in contact_lists:
            subject_ids = dict({subject: 0})
            for i_sbst0 in range(len(epoch_subsets) - 1):
                for i_sbst1 in range(i_sbst0 + 1, len(epoch_subsets)):
                    rdm_size_, rdm0_, rdm1_, csac_, R0_, R1_, contact_list__ = do_analysis_for_two_epoch_sets(contact_list, subject_ids, i_sbst0, i_sbst1,
                                                                                                              V_SAMP_PER_SEC, SHOW_TIME_PER_CONTACT, False, CORR_WINDOW_SEC,
                                                                                                              AUTO_OR_CROSS_ACTIVATION, CONTACT_SPLIT, PROCESS_QUADS, tfm=None,
                                                                                                              SHOW=False, ccorr_mode=CROSS_SESSION_CMODE)
                    contact_list = contact_list__
                    print(subject, i_sbst0, i_sbst1, len(contact_list__))
        print(subject, np.bincount(stat))
